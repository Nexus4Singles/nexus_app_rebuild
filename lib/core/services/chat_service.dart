...
class ChatService {
  ...
  Future<void> _enforceFreeTierOnSend({
    ...
    await _fs.runTransaction((tx) async {
      ...
      final me = meSnap.data() as Map<String, dynamic>?;

      final chat =
          (me?['chat'] is Map) ? (me!['chat'] as Map) : <String, dynamic>{};
      final freePartner = chat['freeChatPartnerId']?.toString().trim();

      // First-ever send => lock to this receiver
      if (freePartner == null || freePartner.isEmpty) {
        tx.set(meRef, <String, dynamic>{
          'chat': <String, dynamic>{'freeChatPartnerId': receiverId},
        }, SetOptions(merge: true));
        return;
      }

      // Already locked to someone else => block
      if (freePartner != receiverId) {
        throw ChatException(
          'Premium required: you can message only one person for free. Upgrade to message more people.',
        );
      }
    });
  }
      ...
      return await _fs.runTransaction((tx) async {
        ...
        if (!isPremium) {
          ...
          if (freePartner != null && freePartner != b) {
            throw ChatException(
              'Premium required: You can chat with only one person for free. Subscribe to chat with more people.',
            );
          }
