...
class DatingSearchService {
  ...
  bool _isLegacyV1Eligible(Map<String, dynamic> data) {
    ...
    final reg = (data['registration_progress'] ?? '').toString().toLowerCase().trim();
  ...
  String _canonKids(String? v) {
    ...
    if (s == 'no kids' || s == 'no kid' || s == 'no children' || s == 'none') return 'no';
    if (s == 'no') return 'no';
  ...
  String _canonEducation(String? v) {
    ...
    if (s.contains('master') || s.contains('msc') || s.contains('mba')) return 'masters';
    ...
    if (s.contains('secondary') || s.contains('high school')) return 'secondary';
  ...
  String _canonIncome(String? v) {
    ...
    if (s == 'none' || s == 'no' || s.contains('no income') || s.contains('unemploy')) {
      return 'none';
    }
  ...
  bool _eqCanon(String? value, String? selected, String Function(String?) canon) {
  ...
  List<DatingProfile> _step(
    ...
    if (kDebugMode) {
      ...
      final parts = <String>['[DatingSearchService][Step] $label: $before -> ${out.length}'];
  ...
  Future<DatingSearchResult> search({
    ...
    if (genders.isEmpty) return const DatingSearchResult(items: <DatingProfile>[]);
        ...
        if (!isV2Verified) {
          ...
          if (!ok) {
            ...
            if (rejectedSamples.length < 3) {
              rejectedSamples.add({
                'uid': d.id,
                'hasPhotos': (data['photos'] is List) && (data['photos'] as List).isNotEmpty,
                'photosLen': (data['photos'] is List) ? (data['photos'] as List).length : null,
                'profile_completed_on': data['profile_completed_on'],
                'registration_progress': data['registration_progress'],
                'compatibility_setted': data['compatibility_setted'] ?? data['compatibilitySetted'] ?? data['compatibilitysetted'],
    ...
    if (kDebugMode) {
      ...
      if (combined.isNotEmpty) {
        ...
        // ignore: avoid_print
        print('[DatingSearchService] pre-age sample: age=${p0.age}, '
    ...
    if (kDebugMode) {
      // ignore: avoid_print
      print('[DatingSearchService] querySummary: seenDocs=$seenDocs, '
      ...
      if (rejectedSamples.isNotEmpty) {
        // ignore: avoid_print
        print('[DatingSearchService] legacyRejectedSamples=' + rejectedSamples.toString());
      }
    ...
    current = _step(current, 'country', (p) => _matchCountry(p, filters),
    ...
    current = _step(current, 'education', (p) => _matchEducation(p, filters),
    ...
    current = _step(current, 'income', (p) => _matchIncome(p, filters),
    ...
    current = _step(current, 'distance', (p) => _matchDistance(p, filters),
    ...
    current = _step(current, 'marital', (p) => _matchMarital(p, filters),
    ...
    current = _step(current, 'kids', (p) => _matchKids(p, filters),
    ...
    current = _step(current, 'genotype', (p) => _matchGenotype(p, filters),
