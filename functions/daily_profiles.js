...
function isUkLaunchDatePassed(marketData, now = new Date()) {
  const launchDate = marketData?.launchDate?.toDate?.();
  return launchDate instanceof Date && launchDate.getTime() <= now.getTime();
}
...
async function calculateDailyProfiles(req, res) {
  ...
  try {
    ...
    const marketDoc = await db.collection('markets').doc('uk').get();

    if (!marketDoc.exists || !isUkLaunchDatePassed(marketDoc.data())) {
      console.log('[DailyProfiles] UK launch date has not passed; skipping generation.');
      return res.status(200).json({
        success: true,
        skipped: true,
        reason: 'uk_not_launched',
        usersProcessed: 0,
      });
    }
    ...
    for (const userDoc of usersSnapshot.docs) {
      ...
      const minAge = Number(preferences.minAge ?? 21);
      const maxAge = Number(preferences.maxAge ?? 65);
      ...
      const scoredProfiles = allProfiles
        .filter(p => {
          ...
          const profileAge = Number(p.data.age || 0);
          return profileGender === targetGender &&
            p.data.status !== 'disabled' &&
            (profileAge === 0 || (profileAge >= minAge && profileAge <= maxAge)) &&
            !viewedProfileIds.has(p.uid);
        })
        .map(profile => ({
      ...
      const existingDailyProfilesDoc = await dailyProfilesRef.get();
      const existingDailyProfiles = existingDailyProfilesDoc.exists
        ? existingDailyProfilesDoc.data()
        : {};

      writes.push({ ref: dailyProfilesRef, data: {
        profiles: scoredProfiles,
        refreshedAt: admin.firestore.FieldValue.serverTimestamp(),
        userGender: userGender,
        userPreferencesSnapshot: preferences,
        notificationSent: existingDailyProfiles.notificationSent === true,
        notificationSentAt: existingDailyProfiles.notificationSentAt || null,
        morningNotificationSent:
          existingDailyProfiles.morningNotificationSent === true ||
          existingDailyProfiles.notificationSent === true,
        morningNotificationSentAt:
          existingDailyProfiles.morningNotificationSentAt ||
          existingDailyProfiles.notificationSentAt ||
          null,
        eveningReminderSent: existingDailyProfiles.eveningReminderSent === true,
        eveningReminderSentAt: existingDailyProfiles.eveningReminderSentAt || null,
        notificationEligible: scoredProfiles.length > 0,
        profileCount: scoredProfiles.length,
      }});

      usersProcessed++;

      if (usersProcessed % 100 === 0) console.log(`[DailyProfiles] Processed ${usersProcessed} users...`);
    }
 ...
 */
async function sendDailyProfileNotifications(req, res, mode = 'morning') {
  const isReminder = mode === 'reminder';
  const notificationSentField = isReminder
    ? 'eveningReminderSent'
    : 'morningNotificationSent';
  const notificationSentAtField = isReminder
    ? 'eveningReminderSentAt'
    : 'morningNotificationSentAt';
  console.log(
    `📢 [Notifications] Starting ${isReminder ? 'evening reminder' : 'morning notification'}...`,
  );
  ...
  try {
    ...
    const marketDoc = await db.collection('markets').doc('uk').get();
    if (!marketDoc.exists || !isUkLaunchDatePassed(marketDoc.data())) {
      console.log('[Notifications] UK launch date has not passed; skipping notifications.');
      return res.status(200).json({
        success: true,
        skipped: true,
        reason: 'uk_not_launched',
    ...
    for (const userDoc of usersSnapshot.docs) {
      ...
      const profiles = Array.isArray(dailyProfilesData.profiles)
        ? dailyProfilesData.profiles
        : [];
      const alreadyNotified = dailyProfilesData[notificationSentField] === true;
      const notificationEligible = profiles.length >= 1 && profiles.length <= 5;
      ...
      if (isReminder) {
        ...
        const viewedToday = await userRef
          .collection('dailyProfilesViewed')
          .where(
            'viewedAt',
            '>=',
        ...
        if (!viewedToday.empty) {
          console.log(
            `[Notifications] Skipping ${userId} - user already viewed a profile today`,
          );
          notificationsSkipped++;
          continue;
        }
      ...
      await notificationRef.set({
        type: 'daily_profiles',
        title: isReminder
          ? "Have you seen today's profiles?"
          : 'New Profiles Available',
        body: isReminder
          ? 'You have profiles waiting to be discovered today.'
          : "Check out today's profiles",
        payload: {
          type: 'daily_profiles',
      ...
      await userDoc
        ...
        .doc(today)
        .update({
          [notificationSentField]: true,
          [notificationSentAtField]: admin.firestore.FieldValue.serverTimestamp(),
        });
...
async function updateProfilesOnPreferenceChange(data, context) {
  ...
  try {
    ...
    if (!marketDoc.exists || !isUkLaunchDatePassed(marketDoc.data())) {
      throw new functions.https.HttpsError(
        'failed-precondition',
        'UK market has not launched yet'
      );
    }
...
const calculateDailyProfilesScheduled = functions.pubsub
  .schedule('0 0 * * *')
  .timeZone('UTC')
  .onRun(async () => calculateDailyProfiles({}, scheduledResponse()));

const sendDailyProfileNotificationsScheduled = functions.pubsub
  .schedule('0 7 * * *')
  .timeZone('Europe/London')
  .onRun(async () => sendDailyProfileNotifications({}, scheduledResponse(), 'morning'));

const sendDailyProfileReminderScheduled = functions.pubsub
  .schedule('0 18 * * *')
  .timeZone('Europe/London')
  .onRun(async () => sendDailyProfileNotifications({}, scheduledResponse(), 'reminder'));

const calculateDailyProfilesHttp = functions.https.onRequest(
  calculateDailyProfiles,
);
const sendDailyProfileNotificationsHttp = functions.https.onRequest(
  sendDailyProfileNotifications,
);

module.exports = {
  calculateDailyProfiles: calculateDailyProfilesHttp,
  sendDailyProfileNotifications: sendDailyProfileNotificationsHttp,
